...
s_int16 PrintFat32(register DEVICE *dev, register u_int32 partitionStart, register int verbose) 
{
    ...
    printf("    Number of Hidden Sectors in Partition %ld\n", GetLE32(myBuf, 0x1c));
    printf("    32-bit Number of Sectors in Partition %ld\n", GetLE32(myBuf, 0x20));
    ...
    printf("    Flags 0x%x\n", GetLE16(myBuf, 0x28));
    printf("    Version of FAT32 Drive 0x%x\n", GetLE16(myBuf, 0x2a));
    printf("    Cluster Number of the Start of the Root Directory %ld\n", GetLE32(myBuf, 0x2c));
    printf("    Sector Number of the FileSystem Information Sector %d\n", GetLE16(myBuf, 0x30));
    printf("    Sector Number of the Backup Boot Sector %d\n", GetLE16(myBuf, 0x32));
    printf("    Logical Drive Number of Partition %d\n", GetLE16(myBuf, 0x40));
    printf("    Extended Signature (0x29) 0x%x\n", GetBE8 (myBuf, 0x42));
    printf("    Serial Number of Partition 0x%04lx\n", GetLE32(myBuf, 0x43));
    ...
    puts("FAT32 SECTOR 1:");
    printf("    First Signature (0x52 0x52 0x61 0x41) 0x%02x 0x%02x 0x%02x 0x%02x\n", GetBE8(myBuf, 0), GetBE8(myBuf, 1), GetBE8(myBuf, 2), GetBE8(myBuf, 3));
    printf("    Signature of FSInfo Sector (0x72 0x72 0x41 0x61) 0x%02x 0x%02x 0x%02x 0x%02x\n", GetBE8(myBuf, 0x1e4), GetBE8(myBuf, 0x1e5), GetBE8(myBuf, 0x1e6), GetBE8(myBuf, 0x1e7));
    printf("    Number of Free Clusters 0x%08lx\n", GetLE32(myBuf, 0x1e8));
    printf("    Most Recently Allocated Cluster 0x%08lx\n", GetLE32(myBuf, 0x1ec));
    printf("    Boot Record Signature (0x55 0xaa) 0x%02x 0x%02x\n", GetBE8(myBuf, 0x1fe), GetBE8(myBuf, 0x1ff));
...
ioresult main(char *parameters) 
{
    ...
    for (i=0; i<nParam; i++) 
    {
        ...
        if (!strcmp(p, "-h")) 
        {
            puts("Usage: FatInfo x: [blkNum] [-v|+v|-h]");
            puts("x:\tDrive name");
            puts("blkNum\tPrint contents of a disk block");
            puts("-v|+v\tVerbose on/off");
    ...
    else if (GetBE16(myBuf, 0x1fe) == 0x55aa) 
    {
        ...
        for (i=0; i<4; i++) 
        {
            ...
            if (t32) 
            {
                static const char *partTypes[] = { "empty", "FAT12", "XENIX root", "XENIX usr",
