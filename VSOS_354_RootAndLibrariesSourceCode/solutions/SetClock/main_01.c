...
auto s_int16 MySetClockSpeed(register u_int32 clkHz) {
  ...
  PERIP32(TIMER_T0L) = (clockSpeed.extClkKHz>>1)-1;
  ...
  if (clkHz <= (s_int32)clockSpeed.extClkKHz*500) {
    ...
    PERIP32(TIMER_T0L) = (clockSpeed.extClkKHz>>(1+shiftRight))-1;
...
void GetVoltages(void) {
  ...
  mV[MV_CVDD]  = 1275 + 25*voltageValues[MV_CVDD];
...
void SetVoltages(void) {
#if 0
  voltageValues[MV_CVDD]  = Sat5B((mV[MV_CVDD] -1240+30/2)/30);
#else
  voltageValues[MV_CVDD]  = Sat5B((mV[MV_CVDD] -1275+25/2)/25);
#endif
