...
u_int32 iq_levels[206*2];
__mem_y u_int32 pi_levels[206*2];
...
void InitFM(void) {
	...
	SetFMFreq(93700);
#if 0
	PERIP(INT_ENABLE1_LP) |= INTF1_RDS;
#endif
}


#if 0
u_int16 SetFM(u_int32 fmFreq) {
	const u_int32 clk = 12288; // in kHz, as is fmFreq
	u_int16 fmDiv = 0;
	u_int16 fmDivBit = 0;
	
	// obtain FMCCF_reg
	// Datasheet: 11.9.2
	// FMCCF_reg = ((F_fm / F_xtal * FM_div - 4*VCO_div) -16) * 2^21
...
#if 0
// Added: Thu Jun 21 10:04:39 EEST 2012
// 
// Samples given sar channel avgcnt times and returns avegered data
// (avgcnt times samples / avgcnt). avgcnt must be between 1 - 63
// 05F/05G: 12: aux0,        7: aux1,  0: aux2, 2: aux3,  4: aux4,
//           6: RTCvdd,      5: cvdd,  3: vco,  1: DLL,  10: VHIGH_div,
//d         11: ??(floating))
u_int16 SARChanAvg(u_int16 chan, u_int16 avgcnt) {
	u_int16 cnt, i;
	u_int16 sarsamples = 0;
	cnt = avgcnt;
	if (avgcnt > 63) {
		cnt = 63;
	}
	...
	// configure SAR
	PERIP(ANA_CF1)  |= (1<<8);      // pdown, ANA_CF1_SARENA;
	PERIP(SAR_CF)     =  0;
	//PERIP(SAR_CF)     = ((chan)<<8) | (3<<6) | 0x07; // fastest, continuous, go
	//DEF//
	PERIP(SAR_CF) = ((chan)<<8)|(3<<6)|0x07; // +7 cycles, continuous, go
	//PERIP(SAR_CF) = ((chan)<<8) | (3<<6) | 0x3F; // +x cycles, continuous, go
	// scrap first data
	while (!(PERIP(INT_ORIGIN1) & INTF1_SAR))          //  int
	;
	PERIP(INT_ORIGIN1) = INTF1_SAR;
	// scrap 2nd data
	while (!(PERIP(INT_ORIGIN1) & INTF1_SAR))          //  int
	;
	PERIP(INT_ORIGIN1) = INTF1_SAR;
	for (i=1; i<=cnt; i++ ) {
		...
		;
		PERIP(INT_ORIGIN1) = INTF1_SAR;
		sarsamples += PERIP(SAR_DAT);
		// printf("  %d", PERIP(SAR_DAT));
	}
	// printf("  <<< %d\n", sarsamples);
	PERIP(SAR_CF)    = 0;          // disable
	PERIP(ANA_CF1) &= ~(1<<8);    // disable
	sarsamples = sarsamples / cnt;
	// printf("\n  SAR chan %d: %d\n", chan, sarsamples);
	PERIP(INT_ORIGIN1) = INTF1_SAR;
	return sarsamples;
}
...
u_int16 SetFMFreq(register u_int32 fmband) {
  ...
  for (cntr=15; cntr>=0; cntr--) {
    ...
    vcolck  = CheckRFLck(0);           // param: printena
#if 0
    printf("%d", vcolck);
#endif
    if (vcolck == 1) {
