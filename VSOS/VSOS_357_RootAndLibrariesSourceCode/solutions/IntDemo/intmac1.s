...
/*
   MAC1 interrupt routine.
   Never call explicitly!
   This routine calls void IntMac1C(void);

   Set the interrupt vector from C as follows:

   extern u_int16 IntMac1Vector;
   [...]
   WriteIMem((void *)(0x20+INTV_MAC1), ReadIMem((void *)(&IntMac1Vector)));

   After setting the interrupt vector, activate the interrupt as follows
     (example for interrupt priority 1):
   PERIP(INT_ENABLE0_LP) |= INTF_MAC1;
   PERIP(INT_ENABLE0_HP) &= ~INTF_MAC1;

   Before exiting your application, deactivate the interrupt as follows:
   PERIP(INT_ENABLE0_LP) &= ~INTF_MAC1;
   PERIP(INT_ENABLE0_HP) &= ~INTF_MAC1;
*/
	.sect code,IntMac1Vector
	.export _IntMac1Vector
_IntMac1Vector:
	jmpi _IntMac1Asm,(i6)+1

	.sect code,IntMac1Asm
	.export _IntMac1Asm
_IntMac1Asm:
