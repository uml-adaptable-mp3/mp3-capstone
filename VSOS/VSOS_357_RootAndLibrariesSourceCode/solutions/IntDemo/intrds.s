...
/*
   RDS interrupt routine.
   Never call explicitly!
   This routine calls void IntRdsC(void);

   Set the interrupt vector from C as follows:

   extern u_int16 IntRdsVector;
   [...]
   WriteIMem((void *)(0x20+INTV_RDS), ReadIMem((void *)(&IntRdsVector)));
   ...
   PERIP(INT_ENABLE1_HP) &= ~INTF1_RDS;

   Before exiting your application, deactivate the interrupt as follows:
   PERIP(INT_ENABLE1_LP) &= ~INTF1_RDS;
   PERIP(INT_ENABLE1_HP) &= ~INTF1_RDS;
*/
	.sect code,IntRdsVector
	.export _IntRdsVector
_IntRdsVector:
	jmpi _IntRdsAsm,(i6)+1
