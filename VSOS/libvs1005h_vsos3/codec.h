...
/**
   ...
   <OL>
	...
	<LI>0x0225 Matrixer removed, audio buffer request added
	<LI>0x0325 Looping added
   </OL>
*/
#define FS_CODEC_SERVICES_VERSION 0x0325
...
#define CFL_SMOOTH_B 0	/**< Prefer smooth looping (not sample accurate) */

#define CFL_SMOOTH 1	/**< Prefer smooth looping (not sample accurate) */


/**
   Codec Loop is activated by placing a pointer to this structure to
   CodecServices.codecLoop. It is deactited by either setting
   CodecServices.cancel (playback stop immediately), or by setting
   numberOfLoops to 0 (playback resumes as normal). The codec
 */
struct CodecLoop {
  ...
  s_int16 work[12];      /**< Internal; clear by the caller before activation */
};
...
struct CodecServices {
  ...
  void (*Spectrum)(struct CodecServices *cs, s_int16 __mem_y *data, s_int16 n,
		   s_int16 ch);
...
#else
  /** Channel matrix (obsolete). Only left so test benches will compile. */
  enum ChannelMatrix matrix[1];
  /** How large audio buffer (in 16-bit words) the codec would like to have. */
  u_int16 audioBufferRequest;
  /** Loop, if requested. */
  struct CodecLoop *loop;
